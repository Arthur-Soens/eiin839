...
namespace Exercice4
{
    ...
    class Client
    {
        ...
        {
            findCloser(args[0], Convert.ToDouble(args[1]), Convert.ToDouble(args[2]));
            Console.ReadLine();

        }

        static async Task findCloser(string contrat, double latitudeA, double longitudeA)
        {
            var point = new GeoCoordinate(latitudeA, longitudeA);
            HttpClient clientSocket = new HttpClient();
            HttpResponseMessage response = await clientSocket.GetAsync("https://api.jcdecaux.com/vls/v3/stations?contract=" + contrat + "&apiKey=2ba463e0d63cedfd5374762396b92c89cd41ec62");
            ...
            Console.WriteLine("Nearest Station - Name : " + chosenOne.name + " Number : " + chosenOne.number + " Latitude : " + chosenOne.position.latitude + " Longitude : " + chosenOne.position.longitude);
        }
    }
}