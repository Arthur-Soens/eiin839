...
namespace RoutingSoapRest
{
    public class RestRouting : IRestRouting
    {
        ...
        public Station[] FindClosestStationFrom(Geometry coordonnee,bool fromTheStart)
        {
            ...
            foreach (Station station in item.stations)
            {
                ...
                try
                {
                    if (fromTheStart && station.totalStands.availabilities.bikes > 0)
                    {
                        dico.Add(me.GetDistanceTo(to), station);
                    }

                    if (!fromTheStart && ((station.totalStands.capacity - station.totalStands.availabilities.bikes) > 0))
                    {
                        dico.Add(me.GetDistanceTo(to), station);
                    }
                }
