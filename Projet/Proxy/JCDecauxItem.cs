    ...
    public class JCDecauxItem
    {
        ...
        public JCDecauxItem(string cacheItemName)
        {
            ...
            else
            {
                ...
                if (key[0] == "All")
                {
                    HttpResponseMessage response = clientSocket.GetAsync("https://api.jcdecaux.com/vls/v3/stations?contract=" + key[1] + "&apiKey=2ba463e0d63cedfd5374762396b92c89cd41ec62").Result;
                    response.EnsureSuccessStatusCode();
                    string responseBody = response.Content.ReadAsStringAsync().Result;
                    stations = JsonSerializer.Deserialize<Station[]>(responseBody);
                }
